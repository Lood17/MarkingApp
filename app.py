...
ACCESS_CODE = os.environ.get("MARKING_APP_PASSCODE", "").strip()

app = Flask(__name__)
app.config["MAX_CONTENT_LENGTH"] = 50 * 1024 * 1024


def valid_access_code() -> bool:
    if not ACCESS_CODE:
        return True
    supplied = (
        request.headers.get("X-Marking-App-Passcode")
...
def require_access_code():
    if request.path.startswith("/api/") and request.path != "/api/config" and not valid_access_code():
        return jsonify({"error": "Enter the teacher access code to use this shared app."}), 401
...
@app.get("/api/config")
def api_config():
    return jsonify({"access_code_required": bool(ACCESS_CODE)})
